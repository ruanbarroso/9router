...
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, customToolNames, trackDone, appendLog, reqTag, log }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      const doneLatency = { total: Date.now() - requestStartTime };
      saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, latency: doneLatency, silent: true });
      if (log?.line) log.line(reqTag, "📊", formatDoneLine({ usage, latency: doneLatency }));
  ...
  try {
    ...
    appendLog({ tokens: usage, status: "200 OK" });
    const totalLatency = Date.now() - requestStartTime;
    saveUsageStats({ provider, model, tokens: usage, connectionId, apiKey, endpoint: clientRawRequest?.endpoint, latency: { total: totalLatency }, silent: true });
